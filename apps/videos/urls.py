...
urlpatterns = patterns(
    ...
    url(r'^diffing/(?P<pk>\d+)/(?P<second_pk>\d+)/$', 'diffing', name='diffing'),
    ...
    url(r'^search-urls/$', 'url_search', name='url-search'),
    ...
    url(r'^(?P<video_id>(\w|-)+)/(?P<lang>[\w\-]+)/(?P<lang_id>[\d]+)/$',
        'subtitles', name='translation_history'),
    url(r'^(?P<video_id>(\w|-)+)/(?P<lang>[\w\-]+)/(?P<lang_id>[\d]+)/(?P<version_id>[\d]+)/$',
        'subtitles', name='subtitleversion_detail'),
